...
<div class="contents eight columns">
    ...
    <form action="{{route('updateSectionC')}}" method="POST" onsubmit="this.querySelectorAll('input').forEach(i => i.disabled = false)">
        ...
        <div class="introPage <x-form-validation-error key='C1'/>" style="margin-bottom: 10px;">
            ...
            <x-ratings-components initialQuestion="0" :completedR="$userProgress[0]->completedR" questionNumber="1" sectionQuestion="C" :userData="$userData[0]->C1" :completedR="$userProgress[0]->completedR"/>
         </div>
         <div class="introPage <x-form-validation-error key='C2'/>" style="margin-bottom: 10px;">
            <p>2. Saya berpuas hati dengan kehidupan saya.</p>
            <x-ratings-components initialQuestion="0" :completedR="$userProgress[0]->completedR" questionNumber="2" sectionQuestion="C" :userData="$userData[0]->C2" :completedR="$userProgress[0]->completedR"/>
         </div>
